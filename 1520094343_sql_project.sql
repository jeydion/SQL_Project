...
SELECT cost.name, (CASE WHEN m.memid = 0 THEN m.surname ELSE CONCAT(m.firstname, ' ', m.surname) END) AS full_name, cost.total_cost
  FROM Members m
  JOIN (SELECT b.facid, b.memid, f.name, SUM(b.slots * (CASE WHEN b.memid = 0 THEN f.guestcost ELSE f.membercost END)) AS total_cost
          FROM Bookings b
          JOIN Facilities f ON b.facid = f.facid
          WHERE LEFT(b.starttime, 10) = '2012-09-14'
          GROUP BY b.memid, b.facid) AS cost
          ON m.memid = cost.memid
  WHERE total_cost > 30
